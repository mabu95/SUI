...
function SetContainerItemLevel(button, ItemLink)
    ...
    if ItemLink then
		name, _, itemRarity, _, _, _, _, _, itemEquipLoc, _, _ = GetItemInfo(ItemLink) name, _, itemRarity, _, _, _, _, _, itemEquipLoc, _, _ = GetItemInfo(ItemLink) itemLevel, _, _ = GetDetailedItemLevelInfo(ItemLink) _, equipped, _ = GetAverageItemLevel()
